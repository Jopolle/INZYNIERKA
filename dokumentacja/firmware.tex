...
Na początku pliku \texttt{main.c} zdefiniowano globalne struktury i zmienne opisujące stan regulatora oraz konfigurację stanowiska. Kluczową rolę pełni struktura konfiguracji HVAC, która zawiera nastawę temperatury, parametry regulatora PI, mapowanie analogowych kanałów wejść i wyjść oraz opis sekwencji grzania i chłodzenia (progi podziału na strefy: chłodzenie, martwą strefę, grzanie, ewentualnie odzysk ciepła/chłodu). Dodatkowe pole tekstowe przechowuje nazwę wariantu sekwencji wykorzystywaną m.in.\ do powiązania konfiguracji z odpowiednią prezentacją graficzną. Aktualnie obowiązująca konfiguracja jest przechowywana w jednej globalnej strukturze, a stan wewnętrzny regulatora w osobnej. Wczytanie nowej konfiguracji polega na skopiowaniu jej do struktury globalnej, zresetowaniu stanu regulatora oraz odświeżeniu odpowiednich elementów interfejsu (dashboard, wizualizator I/O, podgląd sekwencji).


%%%%DO ZMIANY JAK BEDZIE OBSŁUGA SPI

Istotnym elementem są również funkcje abstrakcyjne I/O, które odpowiadają za odczyt napięć z wejść analogowych oraz zapis wartości na wyjściach 0--10~V. Zaprojektowano je jako cienką warstwę pośrednią pomiędzy logiką sterowania a sprzętem, tak aby można było w jednym miejscu przeliczać napięcia na wielkości fizyczne (np.\ temperaturę czy przepływ) oraz odwrotnie. W docelowej wersji projektu te same funkcje opakowują obsługę przetworników A/C i C/A sterowanych magistralami SPI, zachowując prosty, czytelny interfejs od strony regulatora i warstwy HMI.

Cała warstwa HMI oparta jest na obiektach LVGL. W kodzie zadeklarowano zestaw wskaźników na potrzeby poszczególnych ekranów: dashboardu, wizualizatora I/O, ekranu wyboru konfiguracji oraz podglądu sekwencji. Dodatkowe tablice wskaźników na etykiety przechowują obiekty tekstowe reprezentujące kolejne kanały wejść i wyjść analogowych na ekranie wizualizatora. Ekran dashboardu korzysta z etykiety wyświetlającej bieżącą nastawę oraz z obiektów reprezentujących pasma sekwencji, którym przypisane są odpowiednie ikony i przyciski do zmiany progów. Ekran ,,Sequence Viewer'' korzysta z osobnego kontenera, w którym rysowany jest prosty wykres liniowy odzwierciedlający aktualne pasma sekwencji. Współrzędne punktów charakterystycznych (granice martwej strefy, początku i końca pasm grzania, chłodzenia i ewentualnego odzysku) są każdorazowo przeliczane na współrzędne ekranu na podstawie bieżącej konfiguracji, a następnie przekazywane do obiektu LVGL odpowiedzialnego za rysowanie linii. Dzięki temu kształt wykresu zmienia się natychmiast po modyfikacji progów na dashboardzie.

Zasoby graficzne wygenerowane za pomocą konwertera LVGL obejmują przede wszystkim ikony wykorzystywane na dashboardzie: ikonę nastawy temperatury, ikonę płatka śniegu symbolizującą chłodzenie, ikonę grzałki oraz ikonę wymiennika ciepła (odzysk). Każdy z tych plików definiuje strukturę opisu obrazu oraz tablicę danych w formacie wymaganym przez LVGL. W \texttt{main.c} obrazy są jedynie deklarowane i wiązane z obiektami interfejsu; logika HMI operuje więc na wskaźnikach do gotowych zasobów graficznych, a same bitmapy pozostają w osobnych modułach. Wykres sekwencji jest natomiast generowany programowo, dzięki czemu nie wymaga utrzymywania osobnych bitmap dla każdego wariantu pracy.
...
\subsection{Próby wczytywania konfiguracji z karty SD}

Naturalnym uzupełnieniem modelu konfiguracji opartego na JSON było przeniesienie tych plików z kodu
programu do zewnętrznej pamięci, tak aby można było dodawać nowe scenariusze ćwiczeń bez
rekompilacji firmware'u. Najprostszym rozwiązaniem jest karta SD, do której użytkownik może łatwo wgrać
kolejne pliki \texttt{.json} z poziomu komputera. Przykładem takiego podejścia są drukarki~3D i maszyny
CNC, które korzystają z kart SD do przechowywania plików G-code opisujących zadania do wykonania.

Na potrzeby pracy przygotowano wstępną konfigurację takiego rozwiązania. Koncepcja zakłada
wykorzystanie kontrolera karty SD dostępnego na płytce STM32F746G-DISCO i jego opis w DeviceTree,
tak aby Zephyr mógł zainicjalizować urządzenie przy starcie systemu. W warstwie aplikacyjnej
przewidziano prostą procedurę montowania systemu plików FAT na karcie oraz funkcje, które otwierają
pliki JSON o z góry ustalonych nazwach, wczytują ich zawartość do bufora i przekazują ją do istniejącej
funkcji dekodującej konfigurację. Z punktu widzenia interfejsu HMI ekran ,,Config Loader'' miałby zyskać
dodatkowe przyciski pozwalające na wybór pliku z listy dostępnych konfiguracji.

Szczegółowy status realizacji tej funkcjonalności oraz planowane dalsze prace opisano w podrozdziale\ref{sec:kierunki-rozwoju}.
...
\end{center}
Na potrzeby implementacji przyjęto uproszczony model czujnika: napięcie na wejściu analogowym jest liniowo przeliczane na temperaturę (1~V odpowiada 5~\si{\celsius}). Taki model jest wystarczający do uruchomienia i przetestowania algorytmu sterowania oraz do realizacji ćwiczeń laboratoryjnych, a w razie potrzeby może zostać zastąpiony charakterystyką konkretnego przetwornika lub czujnika temperatury stosowanego w rzeczywistej instalacji HVAC.
...
\end{center}
z dodatkowymi ograniczeniami do zakresu 0-10~V. Jeżeli w konfiguracji dane wyjście jest oznaczone jako nieużywane (np.\ wartością \texttt{-1}) albo jego numer wykracza poza zakres dostępnych kanałów, sygnał nie jest w ogóle wysyłany na przetwornik C/A. nastawa sterowania falowników wentylatorów jest niezależna od sygnałów sterujących sekwencjami grzania i chłodzenia. Dobierana jest na podstawie wymagań ilości wymiany powietrza w obiekcie przy znajomości wydatku nominalnego centrali wentylacyjnej.
...
Interfejs użytkownika został zbudowany w całości w bibliotece LVGL i podzielony na kilka logicznych ekranów: dashboard z nastawami i pasmami sekwencji, wizualizator wejść/wyjść analogowych, prosty ,,loader'' konfiguracji oraz ekran z wykresem aktualnej sekwencji. Wszystkie ekrany mają wspólny pasek nawigacyjny u góry z przyciskami ,,Dashboard'', ,,I/O'', ,,Sequence'' oraz ,,Config Loader''. Niezależnie od tego, który ekran jest aktualnie widoczny, użytkownik zawsze ma do dyspozycji ten sam układ przycisków służących do przełączania widoków.

Najważniejszym ekranem z punktu widzenia ćwiczenia jest dashboard. W jego górnej części umieszczono wiersz z przyciskami ,,Screen~1'' i ,,Screen~2'', które przełączają widok między dwiema podstronami. Pierwsza prezentuje nastawę temperatury oraz pasma sekwencji dla chłodzenia i grzania, druga -- pasma martwej strefy i, jeżeli jest w konfiguracji, odzysku ciepła. Przykładowe widoki dashboardu dla dwóch konfiguracji pokazano na rysunkach~\ref{fig:gui-dashboard-screen1-config1} i~\ref{fig:gui-dashboard-screen1-config2}, natomiast rysunek~\ref{fig:gui-dashboard-screen2} obrazuje drugi ekran z ustawieniami martwej strefy i odzysku.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/dashboard12.png}
  \caption{Ekran ,,Dashboard'' (Screen~1) dla pierwszej konfiguracji: nastawa 21~\textcelsius{} oraz pasma grzania i chłodzenia zgodne z konfiguracją JSON nr~1.}
  \label{fig:gui-dashboard-screen1-config1}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/dashboard21.png}
  \caption{Ekran ,,Dashboard'' (Screen~1) dla drugiej konfiguracji: nastawa 23~\textcelsius{} oraz zmienione granice pasm grzania i chłodzenia.}
  \label{fig:gui-dashboard-screen1-config2}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/dashboard22.png}
  \caption{Ekran ,,Dashboard'' (Screen~2): edycja martwej strefy oraz, w razie potrzeby, pasma odzysku ciepła.}
  \label{fig:gui-dashboard-screen2}
\end{figure}
...
Poniżej nastawy znajdują się wiersze opisujące poszczególne pasma sekwencji. Każdy wiersz składa się z ikony (np.\ płatka śniegu dla chłodzenia, symbolu grzałki dla ogrzewania czy wymiennika ciepła dla odzysku) oraz kontrolek pozwalających na zmianę progów \texttt{from} i \texttt{to} w danym paśmie. Progi te są edytowane bezpośrednio z poziomu interfejsu za pomocą przycisków ,,-'' i ,,+'', a informacje o tym, który przycisk zmienia jaki próg i w jakim paśmie, przechowywane są w małej strukturze kontekstu powiązanej z danym przyciskiem. Algorytm modyfikujący wartości pilnuje, aby progi nie wyszły poza zakres \mbox{-100~\%}-\mbox{100~\%} i aby dolna granica nie przekraczała górnej. Po każdej zmianie odświeżane są etykiety z wartościami progów, co dobrze widać na porównaniu rysunków~\ref{fig:gui-dashboard-screen1-config1} i~\ref{fig:gui-dashboard-screen1-config2}.

Kolejny ekran to wizualizator wejść i wyjść układu. W jego górnym wierszu znajdują się przyciski ,,Inputs (AI)'' i ,,Outputs (AO)'', które przełączają widoczność dwóch kontenerów z danymi. Widok wejść pokazuje listę kanałów analogowych 0--7, każdy w osobnym wierszu z numerem kanału, aktualną wartością napięcia oraz opisem roli wynikającym z konfiguracji (np.\ ,,T~supply'', ,,T~extract'' albo ,,Unused''). Odpowiada temu zrzut ekranu na rysunku~\ref{fig:gui-io-inputs}. Widok wyjść jest zbudowany podobnie, lecz dla lepszej czytelności wyjścia podzielono na dwie kolumny, tak aby wszystkie osiem kanałów zmieściło się jednocześnie na ekranie (rysunek~\ref{fig:gui-io-outputs}). Aktualne napięcia na wejściach i wyjściach są odświeżane cyklicznie (co około sekundę), dzięki czemu wizualizator pełni rolę prostego, graficznego miernika pracy przetworników A/C i C/A. Po zmianie konfiguracji role kanałów aktualizują się automatycznie, ponieważ opisy są wyznaczane na podstawie bieżącego mapowania z modelu konfiguracji.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/inputs.png}
  \caption{Ekran ,,I/O Visualiser'' -- podgląd kanałów wejściowych AI wraz z opisami ról przypisanych w konfiguracji.}
  \label{fig:gui-io-inputs}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/outputs.png}
  \caption{Ekran ,,I/O Visualiser'' -- podgląd kanałów wyjściowych AO w dwóch kolumnach.}
  \label{fig:gui-io-outputs}
\end{figure}

Trzeci ekran, oznaczony w interfejsie jako ,,Config Loader'', służy do szybkiego przełączania się między przygotowanymi wcześniej konfiguracjami JSON. W centralnej części ekranu umieszczono dwa duże przyciski, odpowiadające dwóm przykładowym konfiguracjom, oraz etykietę z tekstem statusu (rysunek~\ref{fig:gui-config-loader}). Naciśnięcie przycisku powoduje wywołanie procedury wczytującej konfigurację w formacie JSON i zastosowanie nowego zestawu parametrów w globalnej strukturze konfiguracji. Jeżeli operacja się powiedzie, etykieta statusu informuje użytkownika, że dana konfiguracja została załadowana pomyślnie; w przeciwnym razie pojawia się komunikat o błędzie parsowania. W ten sposób student może w trakcie ćwiczenia łatwo przełączyć się między dwoma scenariuszami bez ponownego programowania mikrokontrolera.

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/loader.png}
  \caption{Ekran ,,Config Loader'' z przyciskami wczytującymi dwie przykładowe konfiguracje JSON oraz etykietą statusu.}
  \label{fig:gui-config-loader}
\end{figure}

Ostatni ekran to ,,Sequence Viewer''. Jego zadaniem jest graficzna prezentacja sekwencji pracy układu na podstawie aktualnych progów zapisanych w konfiguracji. W odróżnieniu od pierwotnej wersji, w której wykorzystywano statyczne bitmapy LVGL, obecna implementacja rysuje wykres liniowy dynamicznie: po każdej zmianie progów na dashboardzie obliczane są nowe współrzędne punktów podziału skali \mbox{-100~\%}-\mbox{100~\%} i aktualizowany jest zestaw linii reprezentujących pasma chłodzenia, martwej strefy, odzysku i grzania. Przykładowe przebiegi dla dwóch wariantów konfiguracji pokazano na rysunkach~\ref{fig:gui-sequence-cool-dead-heat} i~\ref{fig:gui-sequence-rec}. Pierwszy odpowiada prostej sekwencji typu \texttt{cool\_dead\_heat} z wyraźnym pasmem martwej strefy między obszarem chłodzenia i grzania, drugi -- sekwencji z odzyskiem, w której dochodzą dodatkowe ramiona odpowiadające pracy wymiennika.

Ekran ten skupia się na zależności pomiędzy parametrami sekwencji a zachowaniem bloków grzania, chłodzenia i odzysku: zmiana progów na dashboardzie natychmiast przekłada się na zmianę kształtu wykresu w ,,Sequence Viewerze'', co dobrze ilustruje sposób podziału skali od \mbox{-100~\%} do \mbox{100~\%} na poszczególne pasma.


\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/sequence1.png}
  \caption{Ekran ,,Sequence Viewer'' dla konfiguracji typu \texttt{cool\_dead\_heat}.}
  \label{fig:gui-sequence-cool-dead-heat}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/sequence3.png}
  \caption{Ekran ,,Sequence Viewer'' dla konfiguracji z odzyskiem (\texttt{cool\_rec\_dead\_rec\_heat})bez zmian}
  \label{fig:gui-sequence-rec}
\end{figure}

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.75\textwidth]{img/sequence2.png}
  \caption{Ekran ,,Sequence Viewer'' dla konfiguracji z odzyskiem (\texttt{cool\_rec\_dead\_rec\_heat}) ze zmienionym zakresem odzysku chłodu.}
  \label{fig:gui-sequence-rec}
\end{figure}
...
Architektura wątków została dobrana tak, aby logika sterowania mogła w prosty sposób zostać powiązana zarówno z docelowymi przetwornikami A/C i C/A, jak i ewentualnymi interfejsami komunikacyjnymi. W razie potrzeby Zephyr pozwala wydzielić obsługę dodatkowych urządzeń do osobnych zadań, nadać im odpowiednie priorytety i dodać mechanizmy synchronizacji, bez konieczności przebudowy istniejącego kodu regulatora.
...
W firmware przyjęto prosty model abstrakcji wejść/wyjść. W firmware przyjęto prosty model abstrakcji wejść/wyjść. Dla każdego kanału wejściowego AI dostępna jest funkcja zwracająca napięcie 0--10~V w postaci liczby zmiennoprzecinkowej, a dla każdego kanału wyjściowego AO funkcja przyjmująca żądane napięcie w tym samym zakresie. Kod regulatora posługuje się wyłącznie tymi prostymi interfejsami. Przeliczenie napięć na temperaturę i odwrotnie jest zrealizowane w jednym miejscu, według przyjętego liniowego modelu (1~V odpowiada 5~\si{\celsius}). Zmiana typu czujnika temperatury w przyszłości, na przykład na przetwornik z wyjściem 0--10~V o innej charakterystyce, wymaga więc tylko modyfikacji tej funkcji, a nie całego algorytmu sterowania.

W docelowej wersji projektu implementacja warstwy I/O zostanie rozszerzona o wywołania driverów korzystających z magistral SPI skonfigurowanych w DeviceTree. Dla przetwornika A/C będzie to typowy schemat: rozpoczęcie konwersji, odczyt próbki, przeliczenie na napięcie oraz ewentualna filtracja; dla przetwornika C/A odpowiednio: przeskalowanie żądanego napięcia na kod cyfrowy i przesłanie go interfejsem SPI.
...
Kolejnym obszarem, w którym rozwiązanie pozostaje celowo proste, jest warstwa wizualizacji sekwencji. Obecny ,,Sequence Viewer'' rysuje wykres dynamicznie na podstawie aktualnych progów, co w zupełności wystarcza do pokazania idei sekwencyjnego podziału skali \mbox{-100~\%}-\mbox{100~\%}. Wykres nie prezentuje jednak ani historii pracy regulatora, ani wartości wyjść w funkcji czasu. Rozszerzenie go o podświetlanie bieżącego punktu pracy oraz proste logowanie kilku ostatnich sekund sterowania byłoby naturalnym krokiem rozwojowym.
...
Interesującym kierunkiem rozwoju jest także dalsze rozwinięcie widoku sekwencji. Ponieważ wykres jest już rysowany dynamicznie w oparciu o aktualne progi, powinno być możliwe dodanie do niego podświetlanie aktywnego pasma, wskaźnik sygnału regulatora czy prostą animację przejść pomiędzy stanami. Takie rozwiązanie byłoby przejrzyste dydaktycznie i mogłoby pomóc studentom lepiej zrozumieć zależności pomiędzy nastawami a zachowaniem układu. W dalszej perspektywie możliwe jest również dołączenie prostego interfejsu sieciowego, który pozwoliłby na podgląd i modyfikację nastaw z poziomu przeglądarki lub zewnętrznej aplikacji, korzystając z wbudowanych w Zephyra stosów komunikacyjnych.
