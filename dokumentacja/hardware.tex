...
Tor wejściowy odpowiada za obłużenie czujników przemysłowych (pasywnych i aktywnych) i bezpieczne doprowadzenie sygnałów \mbox{0--10~V} do przetwornicy A/C \texttt{ADS8688}. Sygnał cyfrowy \emph{SPI} następnie jest doprowadzany na \emph{GPIO} mikrokontrolera.
...
Wykorzystano wewnętrzną referencję przetwornika: wyprowadzenia \texttt{REFCAP}/\texttt{REFIO}/\texttt{REFGND} są odsprzęgnięte kondensatorami klasy X7R (\textbf{C11X7R} \(1\,\mu\mathrm{F}\) pomiędzy \texttt{REFCAP} i \texttt{REFGND} oraz \textbf{C12}, \textbf{C13} po \(1\,\mu\mathrm{F}\) przy \texttt{REFIO}), zgodnie z zaleceniami producenta dla uzyskania niskoszumowego, stabilnego odniesienia.
